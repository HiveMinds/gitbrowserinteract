...
def remove_previous_github_ssh_key(hardcoded,website_controller):
    ...
    github_ssh_key_exists,link =github_ssh_key_description_exists(hardcoded,website_controller)
    if github_ssh_key_exists:

        # Delete the GitHub personal access token.
        delete_github_ssh_key(link,hardcoded,website_controller)
            
    # Verify token is deleted.
    if github_ssh_key_description_exists(hardcoded,website_controller)[0]:
        raise Exception("Error, GitHub ssh_key is not deleted succesfully.")
        

def github_ssh_key_description_exists(hardcoded,website_controller):
    """Assumes the user is logged in into GitHub. Then lists the already 
    existing GitHub personal access token (PAT) descriptions. If the new GitHub
    ...
    website_controller.driver = open_url(
        ...
        hardcoded.github_ssh_key_tokens_url,
    )
    ...
    elems = website_controller.driver.find_elements(By.CSS_SELECTOR,f".{hardcoded.github_ssh_key_description_elem_classname} [href]")
    for elem in elems:
        link=elem.get_attribute('href')
        if hardcoded.github_ssh_key_description in elem.text:
            return True, link
    return False, None

            

def delete_github_ssh_key(link,hardcoded,website_controller):
    ...
    if link[:len(hardcoded.github_ssh_key_tokens_url)] == hardcoded.github_ssh_key_tokens_url:
        github_ssh_key_id=int(link[len(hardcoded.github_ssh_key_tokens_url):])
        print(f'github_ssh_key_id={github_ssh_key_id}')

        # Get the right table row nr.
        valid_indices=list_of_valid_xpath_indices([],f"{hardcoded.github_ssh_key_table_xpath}/div[","]",website_controller)
        ...
        raise Exception(f'{link[:len(hardcoded.github_ssh_key_tokens_url)]} is not:{hardcoded.github_ssh_key_tokens_url}')
...
def get_desired_token_index(hardcoded,website_controller,valid_indices:List[int]):
    ...
    for row_nr in valid_indices:
        row_elem = website_controller.driver.find_element(By.XPATH,
            f"{hardcoded.github_ssh_key_table_xpath}/div[{row_nr}]"
            )
...
def click_github_ssh_key_delete_button(hardcoded,website_controller,row_nr:int):
    ...
    delete_button  = website_controller.driver.find_element(By.XPATH,
            f"{hardcoded.github_ssh_key_table_xpath}/div[{row_nr}]/div/div[1]/details/summary"
            )
    delete_button.click()

    confirm_deletion_button = website_controller.driver.find_element(By.XPATH,
            f"{hardcoded.github_ssh_key_table_xpath}/div[{row_nr}]/div/div[1]/details/details-dialog/div[4]/form/button"
            )
